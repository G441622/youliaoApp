...
export default class VideoPlayer extends Component {
    ...
    constructor( props ) {
        ...
        this.player = {
            ...
            seekbarTrackPanResponder: PanResponder,
            controlTimeout: null,
            volumeWidth: 150,
            iconOffset: 7,
            seekWidth: 0,
    ...
    */
    ...
    _onLoad( data = {} ) {
        ...
        this.setState( state );
        // fix for full screen?
        if (this.state.currentTime) this.player.ref.seek(this.state.currentTime)
    ...
    _onProgress( data = {} ) {
        ...
        if ( ! state.seeking && state.getPostImage) {
    ...
    _onEnd() {
        this.seekTo(0);
        if (this.props.onEnd && typeof this.props.onEnd === 'function') this.props.onEnd()
    }
    ...
    */
     ...
     */
    _toggleFullscreen() {
        this.state.isFullscreen?this.player.ref.dismissFullscreenPlayer():this.player.ref.presentFullscreenPlayer()
        let state = this.state;
    ...
    seekTo( time = 0 ) {
        ...
        if (time == 0){
            state.paused = !this.props.repeat;
        }
        this.setState( state );
    }
    ...
    */
    ...
    initSeekbarTrackPanResponder() {
        this.player.seekbarTrackPanResponder = PanResponder.create({
    ...
    */
        ...
        return (
            <View style={ styles.seekbar.container }>
                <View
                    ...
                    // { ...this.player.seekbarTrackPanResponder.panHandlers }
                >
                    <View style={[
                        styles.seekbar.fill,
                        {
     ...
     */
    renderVideo = ()=>{
        // if (this.state.isFullscreen && Platform.OS=='ios') return null;
        return (
            <Video
                { ...this.props }
                ...
                paused={ this.state.getPostImage ? this.state.paused : false}
                muted={ this.state.getPostImage ? this.state.muted : true}
                rate={ this.state.getPostImage ? this.state.rate : 1.0}
    ...
    render() {
        ...
        if (this.state.isFullscreen){
            Container =  Modal;
            props = {visible:true,onRequestClose:()=>{},onShow:()=>{}}
        }
        // this.state.isFullscreen && { width,height } 
        const content = (
            <TouchableWithoutFeedback
