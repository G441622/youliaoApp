...
class NewsReply extends React.Component {
    ...
    componentWillReceiveProps(nextProps){
        if (this.props.updateNewsReply!=nextProps.updateNewsReply&&nextProps.updateNewsReply==this.props.news_id){
            this.props.refreshReply();
            this.props.newsReplyAction.updatedNewsReply()
        }
    ...
    onCommentActivePress() {
        ...
        let news_id = this.props.news_id
        ...
        newsReplyApi.newsReply(params).then(responseJson=>{
            if (responseJson.code==200) this.props.refreshReply()
        },error=>console.log(error))
        this.setState({show:false});
    }
    onUserReplyPress(comment,index){
        ...
        this.props.onUserReplyPress({...comment,news_id:this.props.news_id,islike,likecount})
    }
    render() {
        let newsReply = this.props.newsReply;
        let DataTotal = 0
        let commentContents = (
            <View>
                <Text>暂无评论</Text>
        ...
        // console.log('newsReply : ',newsReply)
        if (newsReply && newsReply.hasOwnProperty('DataTotal') && newsReply.DataTotal>0) {
            DataTotal = newsReply.DataTotal
            commentContents = newsReply.data.map((comment,index)=>{
                //news   islike,likecount
