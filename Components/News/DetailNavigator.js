...
import newsLikeApi from '../../api/news/newsLikeApi';
...
class DetailNavigator extends React.Component {
    constructor(props, context) {
        super(props, context);
        let is_like = _.result(this.props,'newsDetail.is_like',1)
        let is_collection = _.result(this.props,'newsDetail.is_collection',1)
        this.state = {
            show: false,
            is_like,
            is_collection,
    ...
    componentWillReceiveProps(nextProps) {
        if (nextProps.newsDetail) {
            let is_like = _.result(nextProps,'newsDetail.is_like',1)
            let is_collection = _.result(nextProps,'newsDetail.is_collection',1)
            this.setState({ is_like, is_collection })
        }
    }
    onLikePress() {
        newsLikeApi.newsLike(this.props.newsDetail.news_info.news_id).then(responseJson => {
            if (responseJson.code == 200) this.setState({ is_like: this.state.is_like == 0 ? 1 : 0 })
        }, error => console.log(error))
    }
    onCollectionPress() {
        newsCollectionApi.getNewsCollection({ news_id: this.props.newsDetail.news_info.news_id })
            .then(responseJson => {
                if (responseJson.code == 200 || responseJson.code == 201) {
                    responseJson.code == 200 
                    ? this.setState({ is_collection: 0 })
                    : this.setState({ is_collection: 1 })
                    // this.setState({ is_collection: this.state.is_collection == 0 ? 1 : 0 })
                }
            }, error => console.log(error))
    }
    onCommentPress() {
    ...
    onCommentActivePress() {
        ...
        let news_id = this.props.newsDetail ? this.props.newsDetail.news_info.news_id : undefined;
        ...
        newsReplyApi.newsReply(params).then(responseJson => {
            if (responseJson.code == 200) {
                ...
                this.props.addNewsReply(newComment)
            }
        }, error => console.log(error))
        this.setState({ show: false });
    }
    ...
    share = (platform) => {
        let { newsDetail, newsReply } = this.props;
        let title = newsDetail.news_info.news_title;
        let defaultInfo = {
            text: '来自有料客户端',
            image: encodeURI('http://www.ulapp.cc/themes/default/css/bglogo.jpg')
        }
        let text, image
        for (let i = 0; i < newsDetail.news_detail.length; i++) {
            let content = newsDetail.news_detail[i]
            if (content.newsdetail_type == 'image') {
                image = content.newsdetail_newscontent
                break;
            } else if (content.newsdetail_type == 'text') {
                text = content.newsdetail_newscontent
            }
            if (text && image) break;
        }
        ...
        let url = `http://ulapp.cc/index.php?m=default&c=news&a=details&id=${newsDetail.news_info.news_id}`
        UMShare.share(title, text, url, image, platform,
            (code, message) => {
                ...
                if (code==200||code=='分享成功'||message=='分享成功'){
                    ...
                    forumDetailApi.forumShare({action:'news',id: newsDetail.news_info.news_id,third:platformString}).then(responseJson=>{
                        if (responseJson.code==200) return this.props.commonAction.showTip('分享成功')
                    },error=>console.log(error))
                }else{
    ...
    render() {
        let commentTotal = _.result(this.props,'newsReply.DataTotal',0)
        let is_like = _.result(this.props,'newsDetail.is_like',1)
        let is_collection = _.result(this.props,'newsDetail.is_collection',1)
        let news_like = _.result(this.props,'newsDetail.news_info.news_like',0)

        news_like = Math.floor(news_like)
        // console.log('is_like : state->',this.state.is_like,' props->',is_like,' is_collection : state->',this.state.is_collection,' props->',is_collection)
        is_like == this.state.is_like ? news_like : (is_like==0?news_like-=1:news_like+=1)
        if (news_like < 0) news_like = 0
        ...
        return (<View style={styles.bottom}>
            <TouchableOpacity style={styles.separate} onPress={this.onCommentPress}>
                ...
                <Text style={styles.text}>{commentTotal}</Text>
            ...
            <TouchableOpacity style={styles.separate} onPress={this.onLikePress}>
                ...
                <Text style={styles.text}>{news_like == 0 ? '' : news_like}</Text>
                <View style={styles.line}></View>
            </TouchableOpacity>
            ...
            <Modal
                ...
                onShow={() => { }}
                onRequestClose={() => { }} >
                <TouchableOpacity activeOpacity={1} style={styles.modalStyle} onPress={this.onCommentCancelPress}>
                    <View style={{ backgroundColor: 'white', borderWidth: 2, borderColor: '#7f7f7f', borderRadius: 10, marginTop: Platform.OS == 'ios' ? 20 : 0 }}>
            ...
            <Modal
                ...
                onShow={() => { }}
                onRequestClose={() => { }} >
                <TouchableOpacity activeOpacity={1} style={styles.sharemodalStyle} onPress={this.onSharePress}>
