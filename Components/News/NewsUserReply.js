...
import * as newsReplyAction  from '../../actions/news/newsReplyAction';
...
class NewsUserReply extends React.Component {
    ...
    componentDidMount() {
        this.getNewsUserReply()
    }
    componentWillUnmount(){
        let {comment} = this.props.navigation.state.params;
        let {news_id=undefined} = comment;
        news_id!=undefined && this.props.newsReplyAction.updateNewsReply(news_id)
    }
    getNewsUserReply = ()=>{
        // pid(父级评论id) page(请求的页数)  limit(每页请求数量) 
        let {comment} = this.props.navigation.state.params
       /* if (comment.commentchild.DataTotal<10) return;*/
        newsReplyApi.getNewsUserReplyPar({action:'news',pid:comment.comment_id})
            .then(responseJson=>{
                // console.log(responseJson)
                if(responseJson.code==200) this.setState({datapar:responseJson.info})
            },error=>console.log(error))
        newsReplyApi.getNewsUserReply({pid:comment.comment_id,page:1,limit:15})
        .then(responseJson=>{
            // console.log(responseJson)
    ...
    refreshNewsUserReply = ()=>{
        let {comment} = this.props.navigation.state.params
        newsReplyApi.getNewsUserReply({pid:comment.comment_id,page:1,limit:15})
        .then(responseJson=>{
            console.log(responseJson)
            if(responseJson.code==200) {
                this.setState({data:responseJson.info.data})
            }
        },error=>console.log(error))
    }
    onLikePress =(item,index)=>{
        let {comment} = this.props.navigation.state.params;
        let {comment_id,user_id,islike} = item;
        let {news_id} = comment;
    ...
    onCommentActivePress = ()=>{
        ...
        let news_id = this.props.navigation.state.params.comment.news_id;
        ...
        newsReplyApi.newsReply(params).then(responseJson=>{
            if (responseJson.code==200) {
                try {
                    // TODOFIXED: 后台数据库处理有问题 -> pid为header中的comment_id
                    console.log(this.props.userdata)
                    let {avatar,alias} = this.props.userdata;
                    let newData = Object.assign({},responseJson.info,{commented_username:user_name,user_name:alias,user_avatar:avatar,commented_username:comment.user_name,islike:1,likecount:0,time:moment().format('YYYY-MM-DD HH:mm:ss')})
                    let data = [newData,...this.state.data];
                    this.setState({data,show:false,comment:undefined,likecount:[0,...this.state.likecount]})
                } catch (error) {
                    
                }
                // TODOFIXED: 回复后刷新不能获取最新数据
                // this.refreshNewsUserReply()
            }
        },error=>console.log(error))
        this.setState({show:false});
    }
    getMoreReply = (input)=>{
        newsReplyApi.getNewsUserReply(input)
        .then(responseJson=>{
            console.log(responseJson)
    ...
    _onEndReached = ()=>{
        if (this.state.data.length<15*this.state.input.page) return;
        console.log('======== NewsUserReply _onEndReached ========');
        let {comment} = this.props.navigation.state.params;
        const input = {
            pid: comment.comment_id,
    ...
    _renderHeader = ()=>{
        // console.log(this.state.datapar)
        ...
        let like_count = this.state.datapar && this.state.datapar.hasOwnProperty('like_count')?this.state.datapar.like_count:0
        ...
        let islike = this.state.datapar && this.state.datapar.hasOwnProperty('islike')?this.state.datapar.islike:1
        ...
        like_count = Math.floor(like_count)
        like_count += this.state.headerlikecount
                    ...
                    <View style={styles.right_top}>
                        ...
                        <View style={styles.top_icon}>
                            <TouchableOpacity  onPress={()=>this.onLikePress(comment,-1)}>
                                <Text style={styles.top_icontext}><Icon name="thumbs-o-up"  color={islike==0?'#d34d3d':'#aaa'}/> {like_count}</Text>
                            </TouchableOpacity>
    ...
    _renderItem = ({item,index})=>{
        ...
        if (!user_avatar||user_avatar==''||user_avatar==null||typeof(user_avatar)!='string') {
            user_avatar = Images.defaultAvatar;
        }
        else {
            user_avatar = {uri:user_avatar} 
        }
                    ...
                    <View style={styles.right_top}>
                        ...
                        <View style={styles.top_icon}>
                            ...
                            <TouchableOpacity  onPress={()=>this.onCommentPress(usercomment,index)}>
                                <Text style={styles.top_icontext}><Icon name="commenting-o"/> {DataTotal==0?'0':DataTotal}</Text>
                            </TouchableOpacity>
                        </View>
                    </View>
                </View>
...
export default connect(mapStateToProps,mapDispatchToProps)(NewsUserReply);